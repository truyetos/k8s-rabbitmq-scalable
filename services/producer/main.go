...
func init() {
	...
	viper.SetDefault("ROUTING_KEY", "")
	viper.SetDefault("CONTINUOUS", true)
	viper.SetDefault("LIFETIME", 3*time.Second)

	uri = viper.GetString("URI")
	exchange = viper.GetString("EXCHANGE")
	queue = viper.GetString("QUEUE")
	routingKey = viper.GetString("ROUTING_KEY")
	continuous = viper.GetBool("CONTINUOUS")
...
func main() {
	...
	go publish(publishOkCh, confirmsCh, confirmsDoneCh, exitCh)

	if lifetime > 0 {
		Log.Printf("running for %s", lifetime)
		time.Sleep(lifetime)
		Log.Printf("end of lifetime, initiating stop")
		exitCh <- struct{}{}
	}
